...
$i = 1;
while (file_exists("productpage_$filename.php")) {
        if ($i > 2) {
...
$myfile = fopen("productpage_$filename.php", "w") or die("Unable to open file!");
$txt = <<<END
<?php 
...
$sql = "UPDATE products
            SET webpageLink = 'productpage_$filename.php', adminpageLink = 'admin_product_$filename.php'
            WHERE id = $product_id;";
mysqli_query($conn, $sql);
